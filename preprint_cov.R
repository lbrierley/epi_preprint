...
fetch_preprints <- function(query, server, page_size = 250){
  ...
  if (server=="biorxiv"){
    ...
    result_list <- mx_search(query, deduplicate = FALSE)
...
preprint_track <- function(query, server, startdate, enddate = as.Date(Sys.time(), format="%Y-%m-%d"), pointsize=3){
  rxiv <- fetch_preprints(query, server = tolower(server))
